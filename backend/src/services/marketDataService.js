...
export class CandleBuilder {
    ...
    onCandleClosed = null;
    maxCandles;
    constructor(periodMinutes, maxCandles = 500) {
        this.periodMs = periodMinutes * 60 * 1000;
        this.maxCandles = maxCandles;
    }
    processTick(price, volume, timestamp, isDummy = false) {
    ...
    updateCandle(periodStart, open, high, low, close, volume, isDummy = false) {
        if (!this.currentCandle) {
            ...
            if (this.allCandles.length > this.maxCandles)
                this.allCandles.shift();
...
export class MarketDataService {
    ...
    lastTickAgeSec = 0;
    lastMessageAgeSec = 0;
    m1 = new CandleBuilder(1, 1000);
    m5 = new CandleBuilder(5, 6000); // 6000 M5 candles = 500 hours (enough for 500 H1 candles)
    m15 = new CandleBuilder(15);
    h1 = new CandleBuilder(60);
    constructor() {
    ...
    setOnM5Closed(callback) {
        this.onM5Closed = callback;
    }
    setOnM1Closed(callback) {
        this.onM1Closed = callback;
    }
    setOnTickUpdate(callback) {
    ...
    processAllTicks(price, volume, timestamp, isDummy = false) {
    ...
    connectTwelveData() {
        ...
        const isWeekend = () => {
            ...
            if (day === 0 && hour < 22)
                return true; // Sunday before 22:00 UTC (Senin 05:00 WIB)
            if (day === 5 && hour >= 22)
                return true; // Friday after 22:00 UTC (Sabtu 05:00 WIB)
            return false;
        };
        const resetWatchdog = () => {
            if (this.watchdogTimer)
    ...
    async generateFallbackCandles(anchorPrice = 2400.00) {
        ...
        if (needsFetch && config.TWELVEDATA_API_KEY) {
            ...
            try {
                ...
                if (json.values && json.values.length > 0) {
                    ...
                    const fetchedCandles = reversed.map((c) => ({
