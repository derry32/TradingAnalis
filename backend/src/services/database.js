...
export async function insertSystemLog(level, source, message, metadata = {}) {
    ...
    try {
        ...
        if (error)
            console.error('[DB] Failed to insert system log:', error.message);
    }
    catch (err) {
...
export async function updateSignalStatus(dbId, status, hitTime, durationMins, accuracy, pips) {
    if (!config.SUPABASE_URL || !config.SUPABASE_KEY)
        return;
...
export async function fetchSignalsByDate(startDate, endDate) {
    if (!config.SUPABASE_URL || !config.SUPABASE_KEY)
        return [];
    const { data, error } = await supabase
        .from('signals')
...
export async function fetchMonthlyStats(year, month) {
    ...
    for (const row of data) {
        let ext = {};
        try {
            ext = JSON.parse(row.reason);
        }
        catch (_) { }
        if (!ext.finalStatus)
            continue; // Skip sinyal yang masih aktif/belum selesai
        totalSignals++;
